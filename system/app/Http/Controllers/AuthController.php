...
class AuthController extends Controller
{
	...
	function prosesLogin()
	{
		//if (Auth::attempt(['email' => request('email'), 'password' => request('password')])) {
			//$user = Auth::user();
			//if($user->level == 1)return redirect('admin/beranda/Admin')->with('success', 'Login berhasil');
			//if($user->level == 0)return redirect('admin/beranda/Pengguna')->with('success', 'Login berhasil');
